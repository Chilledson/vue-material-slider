(function (global, factory) {
	...
	(factory((global.VueMaterialSlider = {})));
	...
	var GestureConfig = function () {
	  /** List of new event names to add to the gesture support list */

	  function GestureConfig(_hammerOptions) {
	    classCallCheck(this, GestureConfig);
	  ...
	  createClass(GestureConfig, [{
	    ...
	    value: function _createRecognizer(base, options) {
	      ...
	      for (var _len = arguments.length, inheritances = Array(_len > 2 ? _len - 2 : 0), _key = 2; _key < _len; _key++) {
	  ...
	  methods: {
	    ...
	    _updateValueFromPosition: function _updateValueFromPosition(pos) {
	      ...
	      if (percent === 0) {
	        ...
	        var closestValue = Math.round((exactValue - this.curMin) / this.step) * this.step + this.curMin;
	        // The value needs to snap to the min and max.
	        this.val = this._clamp(closestValue, this.curMin, this.curMax);
	      }
	    },
	    _getSliderDimensions: function _getSliderDimensions() {
	      return this.$refs.slider ? this.$refs.slider.getBoundingClientRect() : null;
	    },
	    _clamp: function _clamp(value) {
	      var min = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
	      var max = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 1;

	      return Math.max(min, Math.min(value, max));
	    },
	    _focusHostElement: function _focusHostElement() {
	      this.$refs.slider.focus();
	    },
	    _calculateValue: function _calculateValue(percentage) {
	      return this.curMin + percentage * (this.curMax - this.curMin);
	    },
	    _calculatePercentage: function _calculatePercentage(value) {
	      return ((value || 0) - this.curMin) / (this.curMax - this.curMin);
	    },
	    _emitInputEvent: function _emitInputEvent() {
	      this.$emit('input', this.val);
	    },
	    _emitChangeEvent: function _emitChangeEvent() {
	      this.$emit('change', this.val);
	    },
	    _getDirection: function _getDirection() {
	      return this.$data._dir && this.$data._dir == 'rtl' ? 'rtl' : 'ltr';
	    },
	    _shouldInvertMouseCoords: function _shouldInvertMouseCoords() {
	...
	var Slider = __vue_normalize__({ render: __vue_render__, staticRenderFns: __vue_staticRenderFns__ }, __vue_inject_styles__, __vue_script__, __vue_scope_id__, __vue_is_functional_template__, __vue_module_identifier__, undefined, undefined);
